...
public sealed class PedalSKView : SKCanvasView
{
    ...
    private async Task AnimateTo(float target, int durationMs, CancellationToken ct)
    {
        ...
        for (int i = 1; i <= frames; i++)
        {
            ...
            _pressT = start + (target - start) * (float)i / frames;
    ...
    protected override void OnPaintSurface(SKPaintSurfaceEventArgs e)
    {
        ...
        var accent = isBrake ? new SKColor(0xE8, 0x00, 0x1D) : new SKColor(0x00, 0xA8, 0xFF);

        const float margin = 8f;
        var bounds = new SKRect(margin, margin, W - margin, H - margin);
        float minDim = MathF.Min(bounds.Width, bounds.Height);

        using var outer = MakePedalPath(bounds, 0f);
        using var mid   = MakePedalPath(bounds, minDim * 0.085f);
        using var inner = MakePedalPath(bounds, minDim * 0.170f);
    ...
    {
        outer.GetBounds(out var ob);
        mid.GetBounds(out var mb);
        inner.GetBounds(out var ib);
        ...
        using (var paint = new SKPaint { IsAntialias = true })
        {
            using var sh = SKShader.CreateLinearGradient(
                ...
                new[] { new SKColor(0x2C, 0x2C, 0x2C), new SKColor(0x16, 0x16, 0x16), new SKColor(0x07, 0x07, 0x07) },
                new[] { 0f, 0.45f, 1f },
        ...
        // Outer chrome rim
        using (var rim = new SKPaint { IsAntialias = true, Style = SKPaintStyle.Stroke, StrokeWidth = 1.8f })
        {
            using var sh = SKShader.CreateLinearGradient(
                new SKPoint(ob.Left, ob.Top), new SKPoint(ob.Right, ob.Bottom),
                new[] { new SKColor(0xFF, 0xFF, 0xFF, 55), new SKColor(0xFF, 0xFF, 0xFF, 10) },
                null, SKShaderTileMode.Clamp);
            rim.Shader = sh;
            canvas.DrawPath(outer, rim);
        }
        // Junction shadow: outer → mid
        using (var js = new SKPaint { IsAntialias = true, Style = SKPaintStyle.Stroke, StrokeWidth = 5.5f, Color = new SKColor(0, 0, 0, 210) })
        ...
        using (var paint = new SKPaint { IsAntialias = true })
        {
            using var sh = SKShader.CreateLinearGradient(
                ...
                new[] { new SKColor(0x24, 0x24, 0x24), new SKColor(0x12, 0x12, 0x12), new SKColor(0x06, 0x06, 0x06) },
                new[] { 0f, 0.45f, 1f },
                SKShaderTileMode.Clamp);
            paint.Shader = sh;
        ...
        // Mid chrome rim
        using (var rim = new SKPaint { IsAntialias = true, Style = SKPaintStyle.Stroke, StrokeWidth = 1.4f })
        {
            using var sh = SKShader.CreateLinearGradient(
                new SKPoint(mb.Left, mb.Top), new SKPoint(mb.Right, mb.Bottom),
                new[] { new SKColor(0xFF, 0xFF, 0xFF, 44), new SKColor(0xFF, 0xFF, 0xFF, 8) },
                null, SKShaderTileMode.Clamp);
            rim.Shader = sh;
            canvas.DrawPath(mid, rim);
        }
        // Junction shadow: mid → inner
        using (var js = new SKPaint { IsAntialias = true, Style = SKPaintStyle.Stroke, StrokeWidth = 4.5f, Color = new SKColor(0, 0, 0, 195) })
        ...
        using (var paint = new SKPaint { IsAntialias = true })
        {
            using var sh = SKShader.CreateLinearGradient(
                ...
                new[] { new SKColor(0x1C, 0x1C, 0x1C), new SKColor(0x0E, 0x0E, 0x0E), new SKColor(0x06, 0x06, 0x06) },
                new[] { 0f, 0.45f, 1f },
                SKShaderTileMode.Clamp);
            paint.Shader = sh;
        ...
        using var innerRim = new SKPaint { IsAntialias = true, Style = SKPaintStyle.Stroke, StrokeWidth = 1f, Color = new SKColor(0xFF, 0xFF, 0xFF, 18) };
    ...
    private static void DrawLED(SKCanvas canvas, SKRect bounds, float t, SKColor accent, bool ledOnRight)
    {
        ...
        // Active fill
        using (var ledPaint = new SKPaint { IsAntialias = true })
        {
            using var sh = SKShader.CreateLinearGradient(
        ...
        using var glowPaint = new SKPaint { IsAntialias = true };
        using var glowSh = SKShader.CreateLinearGradient(
    ...
    private static void DrawSpecular(SKCanvas canvas, SKPath outer)
    {
        ...
        using var sh = SKShader.CreateRadialGradient(
            ...
            b.Width * 0.55f,
            new[] { new SKColor(0xFF, 0xFF, 0xFF, 14), SKColors.Transparent },
            null, SKShaderTileMode.Clamp);
        paint.Shader = sh;
        canvas.DrawPath(outer, paint);
    }
